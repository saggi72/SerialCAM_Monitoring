...
class MainWindow(QMainWindow):
    ...
    def _generate_video_filename(self):
        """Generate a filename for the video (.mp4 format)."""
        now = datetime.now()
        # Use mp4 extension
        filename = now.strftime("VID_%Y%m%d_%H%M%S") + ".mp4"
        self.last_video_filename = filename # Store for status updates
        return filename
    ...
    def _stop_recording_base(self, action_type, source):
        ...
        # Immediately update state to prevent further writes
        self.is_recording = False
        self.is_paused = False # Reset pause state

        saved_filepath = ""
        # Get filename *before* clearing it or releasing writer
        log_filename = os.path.basename(self.last_video_filename) if self.last_video_filename else "N/A"

        # --- Release the writer ---
        writer = self.video_writer # Get local ref before clearing member
        if writer:
             self.video_writer = None # Clear object reference
             if writer.isOpened():
                 if action_type == "Save" and self.last_video_filename:
                     saved_filepath = os.path.join(self.save_directory, self.last_video_filename)
                     print(f"Releasing writer for SAVE: {saved_filepath}")
                 else: # Discard
                     print(f"Releasing writer for DISCARD: {log_filename}")
                     # Don't clear last_video_filename here, handled below based on action

                 # Actual release - may take time, especially on save
                 try:
                    writer.release()
                    print(f"VideoWriter ({log_filename}) released.")
                 except Exception as e:
                    print(f"Error releasing writer ({log_filename}): {e}", file=sys.stderr)
             else: print(f"Warning: VideoWriter ({log_filename}) was already closed.")
        else: print("Warning: No video writer object found during stop.")

        # --- Update UI & Log based on action ---
        if action_type == "Save":
             if saved_filepath:
                 # Check if the file *actually* exists after release
                 if os.path.exists(saved_filepath):
                     status_msg = f"Đã dừng & lưu: {log_filename}"
                     log_msg = f"Dừng & Lưu [{source}]: {log_filename}"
                     # REMOVED: QMessageBox.information(self, "Lưu Thành Công", f"Đã lưu video:\n{saved_filepath}")
                     print(f"Video saved successfully: {saved_filepath}") # Console confirmation
                 else:
                     # File doesn't exist - Release might have failed or OS issue
                     status_msg = f"LỖI LƯU: {log_filename}"
                     log_msg = f"Dừng & Lỗi Lưu [{source}]: {log_filename}"
                     QMessageBox.warning(self, "Lưu Thất Bại", f"Không tìm thấy file video sau khi lưu:\n{saved_filepath}")
                     # Keep last_video_filename in case user wants to investigate?
             else: # Should not happen if self.last_video_filename was set
                  status_msg = "Đã dừng (Lỗi tên file?)"; log_msg = f"Dừng & Lưu [{source}]: Lỗi tên file?"
        else: # Discard
             status_msg = "Đã dừng (Không lưu)."
             log_msg = f"Dừng & Hủy [{source}]: {log_filename}"
             self.last_video_filename = "" # Clear filename as it's discarded

        self._update_status(status_msg); self._log_serial(log_msg) # Log the result

        # --- Reset Buttons ---
        # Enable Start only if webcam is still available and running
        webcam_can_run = bool(self.webcam_thread and self.webcam_thread.isRunning())
        self.btn_start_record.setEnabled(webcam_can_run)
        self.btn_pause_record.setEnabled(False); self.btn_pause_record.setText("Tạm dừng")
        self.btn_stop_save_record.setEnabled(False)
        self._update_status_visuals() # Update status label

        # Clear filename if save was successful (or discard)
        if (action_type == "Save" and saved_filepath and os.path.exists(saved_filepath)) or action_type == "Discard":
             self.last_video_filename = ""

        return True # Indicates stop action was processed
